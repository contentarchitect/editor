    ...
    <block
        ...
        <template slot="view">
            <div class="image-block">
                <div v-for="(image, i) in images" :key="i" ref="figure">
...
<script>
import { Block, Button, Editable } from "@contentarchitect/base"
...
export default {
    ...
    methods: {
        renderedHtml () {
            return `<img src="${'https://placeimg.com/640/480/any'}" />`
        },
    ...
    watch: {
        imageCount (newImageCount, oldImageCount) {
            if (newImageCount > oldImageCount) {
                let addCount = newImageCount - oldImageCount;
                this.images.push(...Array(addCount).fill().map(_ => { return { ...this.emptyImage } }));
            } else {
                this.images.splice(newImageCount)
            }
        }
    }
}
